...
def get_s_and_a(df, answer_filename):
    '''Returns source and answer text from passed in df and answer filename
    :param df: A dataframe of file information
    :param answer_filename: the answer filename to be retirieved as the answer text(a)
    :return: two indices a and s containing the indices for the answer and source text'''
    
    task = ''

    for i in range(df.shape[0]):
        if df.loc[i, 'File'] == answer_filename:
            a = i
            task = df.loc[i, 'Task']
        if df.loc[i, 'Task'] == task:
            if df.loc[i, 'Datatype'] == 'orig':
                s = i
